...
#include "general.h"	     /* Widely used definitions, error handling, initialization                                  */
#include "p7_alidisplay.h"	     /* P7_ALIDISPLAY : an alignment formatted for output                                        */
#include "p7_alidisplay_mpi.h"  /*               :    ... add-on: MPI communication                                         */
#include "p7_anchors.h"         /* P7_ANCHORS    : array of (i0,k0) anchors for defining domain locations in target seq     */
#include "p7_anchorhash.h"      /* P7_ANCHORHASH : auxiliary data structure in MPAS (most probable anchor set) algorithms   */
#include "p7_bg.h"		     /* P7_BG         : null model, of an entirely nonhomologous target seq                      */
#include "p7_coords2.h"         /* P7_COORD2, P7_COORDS2 : segment start/end coord pairs, used in domain definition         */  // DEPRECATED; J13/92
#include "p7_domain.h"	     /* P7_DOMAIN     : information about a match to a model in a target seq                     */  // needs to be reconciled with newer P7_ENVELOPES
#include "p7_domain_mpi.h"      /*               :    ... add-on: MPI communication                                         */
#include "p7_envelopes.h"       /* P7_ENVELOPES  : information about locations of domains on a target sequence              */
#include "p7_hmm.h"	     /* P7_HMM        : basic model, probability parameters or counts                            */
#include "p7_hmm_mpi.h"	     /*               :    ... add-on: MPI communication                                         */
#include "p7_hmmfile.h"	     /* P7_HMMFILE    : reading models from files                                                */
#include "p7_hmmwindow.h"	     /* P7_HMM_WINDOW, P7_HMM_WINDOWLIST : {nhmmer}                                              */
#include "p7_masstrace.h"	     /* P7_MASSTRACE  : workspace used in calculating envelope bounds for a domain               */  // DEPRECATED
#include "p7_prior.h"	     /* P7_PRIOR      : Dirichlet mixture prior on model parameters                              */
#include "p7_profile.h"	     /* P7_PROFILE    : search model, glocal/local, with additional states for nonhomologous seq */
#include "p7_profile_mpi.h"     /*               :    ... add-on: MPI communication                                         */
#include "p7_scoredata.h"	     /* P7_SCOREDATA  : {nhmmer}                                                                 */
#include "p7_tophits.h"	     /* P7_HIT, P7_TOPHITS : accumulated information about hits (scores, alis) during search     */
#include "p7_tophits_mpi.h"     /*               :    ... add-on: MPI communication                                         */
#include "p7_trace.h"	     /* P7_TRACE      : alignment of a model to a sequence: an HMM state path                    */

/* 'misc' subdir: various other support functions                                                                         */
#include "emit.h"		           /* emitting (sampling) sequences from HMM or profile                           */
//#include "h2_io.h"		           /* Legacy support for HMMER2 model file formats                                */
//#include "p7_trace_metrics.h"         /* Benchmarking utilities for alignment accuracy                               */
#include "logsum.h"	           /* Fast lookup-table-driven log-sum-exp-2 function                             */
//#include "tracealign.h"	           /* Conversion of trace structures to multiple alignment                        */

/* 'build' subdir: building and calibrating a new model */
//#include "build.h"	           /* Constructing a model from a multiple seq alignment                          */
//#include "seqmodel.h"	           /* Constructing a model from a single query seq                                */
//#include "evalues.h"	           /* Model calibration, setting parameters for E-value determination             */
//#include "build/eweight.h"	           /* Entropy-weighting: ad hoc absolute sequence weights                         */
#include "modelsample.h"	           /* Sampling models randomly; used extensively in unit testing                  */
//#include "modelstats.h"	           /* Various summary statistics for a model                                      */
//#include "p7_builder.h"	           /* P7_BUILDER: an aggregrated pipeline for building new models                 */

/* 'search' subdir: compare a model against a sequence, to calculate score and alignment(s)                               */
#include "modelconfig.h"	           /* Building a search profile (P7_PROFILE) from a basic model (P7_HMM)          */
...
#include "simdvec.h"	           /* general definitions and utilities for SIMD vector code                      */
#include "p7_oprofile.h"	   /* P7_OPROFILE  : striped/vectorized profile                                   */
//#include "p7_oprofile_mpi.h"     /*              :  ... add-on: MPI communication                               */
#include "p7_filtermx.h"	   /* P7_FILTERMX  : a one-row O(M) memory DP matrix for SSV, MSV, VF             */
#include "p7_checkptmx.h"	   /* P7_CHECKPTMX : checkpointed O(M sqrt(L)) memory DP for F/B/Decode           */
