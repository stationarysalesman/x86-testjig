...
/* natural logarithm computed for 4 simultaneous float 
   return NaN for x <= 0
*/
__arm128f log_ps(__arm128f x) {
  __arm128f one, e, tmp, z, y;
  __arm128i invalid_mask, ux, si, mask;

  one.f32x4 = vdupq_n_f32(1);
  x.f32x4 = vmaxq_f32(x.f32x4, vdupq_n_f32(0)); /* force flush to zero on denormal values */
  invalid_mask.u32x4 = vcleq_f32(x, vdupq_n_f32(0));
  ux.s32x4 = vreinterpretq_s32_f32(x);
  emm0.s32x4 = vshrq_n_s32(ux, 23);
  ...
  */
  ...
  y.f32x4 = vmulq_f32(y.f32x4, z);
  

  tmp.f32x4 = vmulq_f32(e, vdupq_n_f32(c_cephes_log_q1));
...
__arm128f exp_ps(__arm128f x) {
  __arm128f tmp, fx, one;
  __arm128i mask;
  one.f32x4 = vdupq_n_f32(1);
...
/* exp() computed for 4 float at once */
__arm128f exp_float(__arm128f x) {
